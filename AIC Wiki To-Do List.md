---
description: "Current page count: ."
---
**Current page count: 1375.**

## Tasks
* Upload [[Performers/Sara Farr|Sara Farr]]'s AIC logo designs.
  * Also, re-save the Illustrator files as JPGs, and upload those.
* Add bit about crediting photos to the [[Performers/AIC Wiki Style Guide|AIC Wiki Style Guide]].
* Infoboxes.
  * Include more thorough information about each field.
  * Make infobox field names link to the documentation about the field.
* Make each infobox include some indication of its category.
  * e.g., a "performer" infobox has the word "Performer" visible somewhere.
    * Maybe in a thing above the main box

## Content
This section is for the most significant missing content on the site.

* Add info from [this thread](http://forum.austinimprov.com/viewtopic.php?t=9591) and [this thread](http://forum.austinimprov.com/viewtopic.php?t=14630).
* Make pages for other festivals.
* Get the history of the Hideout on to its page.
* Finish out pages for the [[Lists/List of Hideout Mainstage Shows in Chronological Order|Hideout mainstage shows]].
* Finish making basic pages for all troupes that have attended out of bounds (and thus have online bios).

testing!