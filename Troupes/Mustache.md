---
description: "An improv troupe."
image: "File/Mustache.jpg"
---
<div>
	<table class="infobox infobox-troupe">
		<tr>
			<th class="infobox-header" colspan="2">Mustache</th>
		</tr>
		<tr class="">
			<td colspan="2" class="infobox-picture">
				<a href="File/Mustache.jpg.md"><a class="internal-link" href="File/Mustache.jpg.md"><img src="File/Mustache.jpg" alt="" style="max-width: 220px; height: auto; display: block; margin: 0 auto;" /></a></a>
			</td>
		</tr>
		<tr class="">
			<th class="category-header" scope="row">Years Active</th>
			<td class="category">2011</td>
		</tr>
		<tr class="">
			<th class="category-header" scope="row">Cast</th>
			<td class="category"><div class="plainlist"><ul style=""><!--
  --><li style=""><a class="internal-link" href="Performers/Ed Melendez">Ed Melendez</a></li><!--
  --><li style=""><a class="internal-link" href="Performers/Frank Netscher">Frank Netscher</a></li><!--
  --><li style=""><a class="internal-link" href="Performers/Kate Bojanek">Kate Bojanek</a></li><!--
  --><li style=""><a class="internal-link" href="Performers/Kristin Henn">Kristin Henn</a></li><!--
  --><li style="">Wes Gandy</li><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
  --><!--
--></ul></div></td>
		</tr>
	</table>
</div>

**Mustache** was an improv troupe.

## Summary
### Press Blurb
Their press blurb, taken from a 2011 application to perform at [[Theatres/The Hideout Theatre|The Hideout Theatre]]:
> Mustache creates character driven, narrative based improv, generating original stories and situations. Each 25 minute show establishes a specific location, and a group of individuals who inhabit that place - together, the performers explore the relationships and events that motivate these characters. Funny and compelling, the characters created within the show are only brought to life for the duration of the show, then they're gone forecer. See them before they disappear into memory...


### "What's Your Deal?"
Their answer to the "What's Your Deal?" question on a 2011 application to perform at [[Theatres/The Hideout Theatre|The Hideout Theatre]]:
> Our format works like this:
> 
> * We ask the audience for a number between 3 and 7, then play songs from a random play list until we reach the suggested number.
> * We use that song for our suggestion - walking around the stage until we find characters.
> * We start the show with a group scene that includes at least most of the characters.
> * We follow with shorter two or three person scenes that build on the world created in the first scene.
> * We punctuate the scenes with one or two "interviews" where one or two characters step out of the action and deliver a monologue directly to the audience. If two characters are sharing this moment, the monologues may overlap, but there is no conversation.
> * We end in a group scene.
> * We do create a story line and often have some kind of resolution at the end. 


## Media
### Videos
* [Video of their 11/7/10 show.](http://vimeo.com/18781674)
* [Video of their 1/13/11 show.](http://vimeo.com/19912376)
* [Video of their 1/24/11 show.](http://vimeo.com/19827335)
* [Video of their 5/18/11 show.](http://vimeo.com/23991678)

## More Information
* [The troupe's facebook page.](http://www.facebook.com/pages/Mustache/169381313078021)


[[Category/Troupes|Category:Troupes]]
[[Category/Auto-Generated Troupe Pages|Category:Auto-Generated Troupe Pages]]